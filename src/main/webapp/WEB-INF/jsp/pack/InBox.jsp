...
<body>
<div id="mainbody">
<form id="packForm" method="post" action="<c:url value="/pack/pack.html"/>">
				...
				<c:if test="${!empty goodses}">
						<table style="width: 700px;" border="1">
						 ...
						 <%--  <c:forEach items="${goodses}" var="goods" varStatus="a"> --%>
						  <tr align="center">
							<td>${a.index+1}</td>
							<td>${goodses.goodsId}</td>
							<td>${goodses.barcodePrint.component.componentId}</td>
							<c:forEach items="${siteColumns}" var="column">
								<td><c:forEach items="${goodses.attachmentParameters}"
							...
							<td><fmt:formatDate value="${goodses.packDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
						  </tr>
						 <%--  </c:forEach> --%>
						</table>
				</c:if>
