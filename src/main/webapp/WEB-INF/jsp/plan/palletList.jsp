...
$(function(){
	$(".goodsButton").click(function(){
		window.location.href = '<c:url value="/plan/getGoodses.html?palletId="/>'+$(this).attr("id");
	});

	$(".updateButton").click(function(){
		window.location.href = '<c:url value="/plan/toUpdatePallet.html?palletId="/>'+$(this).attr("id");
	});
});
</script>
<title>${modelName} 生產管理系統--出貨查询</title>
		...
		<div id="mainbody">
			<c:if test="${!empty pallets}">
				<div></div>
					<table border="1" >
					<caption>
							出貨計畫 ${pallets[0].shipPlan.poplan.casemark2} 出貨棧板列表					
