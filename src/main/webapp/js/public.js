...
function ts_change_dt(ts,format){
	var daycount = parseInt(ts/86400000);
	var year = 1970;
	var month = 1;
	while(daycount >= 365){
		if(checkyear(year)){
	...
	for(var i=0; i<month_arr.length; ++i){
		if(daycount-month_arr[i]>=0){
			if(i != 1){
				daycount = daycount-month_arr[i];
			}else if(i == 1){
				if(checkyear(year)){
	...
	//alert("月份:"+month);
	var day = daycount+day;
	//alert("日期:"+(day));
	
	var seconds = ts%86400000;
	//alert(other);
...
function checkNumber(name, TempS) {
	for(var Count=0;Count<TempS.length;Count++) {
